...
public class AngleNSGAIIManager {
    ...
    public AngleNSGAIIManager(){

        setParameters(AngleNSGAIIManager.getDefaultParams());
    }
    ...
    public AngleNSGAII Create() {
        ...
        domRank.setDominanceComparator(new DominanceComparator<Solution<?>>(RelaxationType.ANGLE, this.angleFactor));
        ...
        return (new AngleNSGAIIBuilder(problem, crossover, mutation)).setSelectionOperator(selection).setMaxEvaluations(maxEvaluations).setAngleFactor(angleFactor).setPopulationSize(populationSize).build();
    }
